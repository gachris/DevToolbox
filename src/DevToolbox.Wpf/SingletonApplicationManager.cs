...
public class SingletonApplicationManager
{
    ...
    /// The WPF <see cref="System.Windows.Application"/> to control (shutdown or dispatcher invoke).
    /// </param>
    ...
    public void Register(System.Windows.Application application, Action onRegistered, Action onInstanceAlreadyRunning)
    {
        ...
        if (onRegistered is null)
        {
            throw new ArgumentNullException(nameof(onRegistered));
        }

        if (onInstanceAlreadyRunning is null)
        {
            throw new ArgumentNullException(nameof(onInstanceAlreadyRunning));
        }
