...
internal static class ViewportHelpers
{
    ...
    /// <returns></returns>
    public static Point? FilterClamp(this Point value, double xMax, double yMax) => (value.X < 0 || value.X > xMax || value.Y < 0 || value.Y > yMax) ? null : value;
    ...
    public static Point Clamp(this Point value, Point topLeft, Point bottomRight) => new Point(
    ...
    {
        return !contentWidth.HasValue || !contentHeight.HasValue
            ? 1
            : Math.Min(actualWidth / contentWidth.Value, actualHeight / contentHeight.Value);
        ;
    }
    ...
    public static double FillZoom(double actualWidth, double actualHeight, double? contentWidth, double? contentHeight)
    {
        return !contentWidth.HasValue || !contentHeight.HasValue
            ? 1
            : Math.Max(actualWidth / contentWidth.Value, actualHeight / contentHeight.Value);
        ;
    }
}
